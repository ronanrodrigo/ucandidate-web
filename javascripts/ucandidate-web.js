angular.module('ucandidate-web', [])
  .controller("CandidateCtrl", ['$http', '$scope', function($http, $scope){
    var self = this;
    ...
    self.submit = function () {
      ...
      $http.post("http://ucandidate-api.herokuapp.com/api/candidates", {"candidate" : self.candidate}).then(function(response){
        nanobar.go(100);
        self.startData();
        $scope.$watch('candidateForm', function(candidateForm) {
          candidateForm.$setPristine();
          candidateForm.$setUntouched();
        });
